...
def min_distance_A():
	...
	while (parent_step.component_left[0]!='' or parent_step.component_left[1]!='' or parent_step.component_left[2]!='' or parent_step.component_left[3]!='' or parent_step.component_left[4]!=''):
		...
		# q1.sort(key = lambda elements: (elements.cost+elements.h))
		parent_step = min(q1, key = lambda x:(x.cost+x.h))
		...
		if(counter % 1000 == 0):
			...
			print parent_step.cost + parent_step.h
...
print "below is the pairwise shortest path calculated by UCS"
for x in shortest_table.itervalues():
	print x.path,x.cost
